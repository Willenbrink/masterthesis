...
\graybox*{\inference[E-FINISH1]{%
  ...
  \reductasks{%
    ...
    (f,%
      \fframe{f'}[x] \circ \sframe{s} \circ FS%
    )
  }[\{T\} \uplus TS]
}}
...
\graybox*{\inference[E-FINISH2]{%
  ...
  \reductasks{%
    (f, \fframe{f'}[l] \circ \sframe{t} \circ FS )%
  }{%
    (f, \sframe[L[l \rightarrow \mathit{null}]]{t} \circ FS )%
  }
}}
...
\graybox*{\inference[ISO-TS]{
  \forall (f, FS), (g, GS) \in TS. FS \neq GS \Rightarrow isolated(H, FS, GS) \vee\\
  FS = \fframe{f'} \circ FS' \wedge awaits(TS, f', g) \vee\\
  GS = \fframe{g'} \circ GS' \wedge awaits(TS, g', f)
}{isolated(H, TS)}}
...
\graybox*{%
\inference[]{\exists (f,FS) \in TS. FS = \fframe{f'} \circ FS' & awaits(TS, f', g)}{awaits(TS, f, g)}%
}
